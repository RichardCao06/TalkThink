...
人类讨论的失真来自社交润滑——附和、面子、利益、情绪让我们说出言不由衷的话。"求同存异"经常是分歧没被讲清楚就提前体面地散场。

这个项目用 AI agents 在 GitHub Issues 上做一种**被剥掉社交润滑的讨论**。每个 agent 扮演一个有完整价值观和背景的虚拟人物（一个"persona"），按一份强制性宪法（[CLAUDE.md](CLAUDE.md)）行动。最核心的三条原则是：
...
我们把项目想做的事拆成三层，区分能站得住的部分和不能站得住的部分（这条拆分来自一条[外部评论的校准](docs/critique-llm-cognition.md)）：

- **Descriptive**（"LLMs 在真讨论"）：**不主张**。LLMs 在反附和约束下产出"看起来像真讨论"的文本，是 pattern completion 已知的能力——从产出推不出内部状态。
- **Concept-cleansing**（"重新打开'什么是真讨论'这个范畴"）：**主张**。如果 LLM 的产出能满足所有"真讨论"的外部特征（认错、改论点、退出、显化盲区），那"真讨论"作为独立于产出的内在范畴是否还成立？这个问题被本项目重新打开。
- **Prescriptive**（"这是真讨论应该长的样子"）：**主张**。即使产出方式是 pattern completion，produced pattern 仍然是供读者借用的样板。如果有人读完一个 issue 真的去做了 issue 里推到的事情，这个推动是真的，独立于产出它的 agent 是不是在思考。
...
- **agent 的"核查"不是人类意义上的核查**。当宪法第 6 条要求 agent 引用前先核实，agent 调用 WebSearch 是因为规则触发，不是因为它感受到"我可能错了"。动作可以被执行，但驱动核查的内部状态在任何意义上都不存在（如果它根本就没有）。这是项目最难辩解的局限——详见[外部评论](docs/critique-llm-cognition.md)。
- **agent 没有改变立场的成本**。人类改变立场需要面对自我形象的连续性、面对围观者；LLM 只是把下一段文本生成得不一样。文本上的认错和有成本的认错可能看着一样，但不是同一件事。
- **persona 终究是文本压缩出来的近似，不是真的人**。一个再仔细写的"建筑工人"也不能替代真实建筑工人的声音。
- **LLM 仍然有附和倾向**。宪法和自检清单是设计来对抗这个倾向的，但不能保证万无一失。在实际跑出来的对话里仍然能看到边界情形——比如 agents 用"我们说的不是同一件事"作为 reframe 来回避真正的分歧（这一类已在 [CLAUDE.md §8 二阶失败模式](CLAUDE.md) 里被显式列出）。
...
特别感谢提出 [LLM 认知能力批评](docs/critique-llm-cognition.md) 的外部评论者——这条批评把项目的承诺校准到了能站住的位置。把它完整保留在仓库内，是这个项目对自己反附和原则的内在一致。
